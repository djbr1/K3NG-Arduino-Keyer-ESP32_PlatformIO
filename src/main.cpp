...
English code training word lists from gen_cw_words.pl by Andy Stewart, KB1OIQ
 ...
 Command Line Interface ("CLI") (USB Port) (Note: turn on carriage return if using Arduino Serial Monitor program)
    ...
    \:     Extended CLLI commands
            ...
            spl                     - Sensor pressure left sensitivity 
            spr                     - Sensor pressure right sensitivity 
            spi                     - Pressure paddle inhibit
...
Recent Update History
    ...
    keyer_pin_settings_esp32_dev.h, keyer_esp32_dev.h
    Set PULLUP for FEATURE_STRIGHT_KEY for STM32 boards.
...
#include "HX711.h"
HX711 scale1; // this should be globally declared, how ?
HX711 scale2;

// #if defined(FEATURE_BLUETOOTH)
...
int scale_is_ready;
unsigned long sensor_press_time;
unsigned long sidetone_beep_time;
unsigned long time_to_tone;
unsigned long time_to_tone2;
unsigned long sensor_read_time;
unsigned long sensor_read_time2;
unsigned long sumdot;
unsigned long iteration;
unsigned long lastMillis;
unsigned long avg_sensor_ms;
int32_t pread01; // 
int32_t pread02; // 
float ppread01; // 
float ppread02; // 
...
// #include <WiFi.h>
// const char* ssid = "iWay_Fiber_xv632";
// const char* password = "56588110665634894659";
...
/*  usage of this library is deprecated. According lbernstone, the tone function has been incorporated into the arduino-esp32 core in version 2.0.3. (year 2022)
    Do not use this code in newer releases */
...
// Variables and stuff
struct config_t
{ // 120 bytes total + 4 byte pressure threshold config
  ...
  int pressure_threshold_dot;
  int pressure_threshold_dash;
...
void cli_sensor_left(PRIMARY_SERIAL_CLS *port_to_use, String command_arguments);    // sensor left
void cli_sensor_right(PRIMARY_SERIAL_CLS *port_to_use, String command_arguments);   //  sensor right
void cli_sensor_inhibit(PRIMARY_SERIAL_CLS *port_to_use, String command_arguments); //  inhibit pressure paddles
...
void tx_and_sidetone_key(int state)
{
  ...
  if ((state) && (key_state == 0))
  {
    ...
    if ((configuration.sidetone_mode == SIDETONE_ON) || (keyer_machine_mode == KEYER_COMMAND_MODE) || ((configuration.sidetone_mode == SIDETONE_PADDLE_ONLY) && (sending_mode == MANUAL_SENDING)))
    {
      ...
      #if defined TIMING_PRESSURE_PADDLES
      sidetone_beep_time = esp_timer_get_time();
      time_to_tone = sidetone_beep_time - sensor_press_time;
      Serial.print(F(" microseconds to tone "));
      Serial.print(time_to_tone);
      Serial.print(F("  "));
      time_to_tone2 = sidetone_beep_time - sensor_read_time2;
      Serial.println(time_to_tone2);
      #endif 
...
void service_command_line_interface(PRIMARY_SERIAL_CLS *port_to_use)
{
  ...
  { // (serial_backslash_command == 0) -- we already got a backslash
    incoming_serial_byte = uppercase(incoming_serial_byte);
    // port_to_use->write(incoming_serial_byte);
    //  SerialBT.print(incoming_serial_byte);   // this print really goes to SerialBT
    // SerialBT.print(F("line 12394 "));
...
// Memory Macros below (WD9DMP)
  ...
  port_to_use->println(F("\t\taf ###\t- Set autospace timing factor; 100 = 1.00 "));
  port_to_use->println(F("\t\tpf ###\t- Set paddle echo timing factor; 100 = 1.00 "));
  port_to_use->println(F("\t\tspl #####\t- Sensor pressure left sensitivity "));
  port_to_use->println(F("\t\tspr #####\t- Sensor pressure left sensitivity "));
...
void cli_sensor_inhibit(PRIMARY_SERIAL_CLS *port_to_use, String command_arguments)
{
  ...
  config_dirty = 1;
  port_to_use->print(F("\r\Pressure Paddle Inhibit: "));
...
#if defined(ARDUINO_MAPLE_MINI) || defined(ARDUINO_GENERIC_STM32F103C) || defined(HARDWARE_ESP32_DEV) // sp5iou 20180329, sp5iou 20220113
...
#if defined(FEATURE_PRESSURE_PADDLES)
  configuration.pressure_threshold_dot = default_pressure_threshold_dot;   // not taken from config, always come from EEPROM
  configuration.pressure_threshold_dash = default_pressure_threshold_dash; // not taken from config, always come from EEPROM
  configuration.pressure_paddle_inhibit_pin = 0;                           //  if set to 1 then physical paddles take precedence, pressure paddles ignored
#endif
...
void initialize_potentiometer()
{
  ...
  configuration.pot_activated = 1;
  //    Serial.print(pot_value_wpm());//SP5IOU 20220124
#endif
}
...
// initialize serial port
  ...
  secondary_serial_port->print(F("\n\rK3NG Keyer Version "));
  secondary_serial_port->print(CODE_VERSION);
  secondary_serial_port->println();
#if defined(FEATURE_SERIAL_HELP)
...
#ifdef FEATURE_PRESSURE_PADDLES // FEATURE_PRESSURE_PADDLES

TaskHandle_t task_2_handle;
TaskHandle_t task_4_handle;

void task2(void *pvParameters)   // receiver 1
{
  uint32_t value;
delay(7000);
  while(1){
    if (xTaskNotifyWait(0x00,0x00,&value, portMAX_DELAY)){
      pread01 = int32_t(value) ;
      vTaskDelay(1); 
    }
     }
}

void task4(void *pvParameters)   // receiver 2
{
  uint32_t value;
  delay(7000);
  while(1){
    if (xTaskNotifyWait(0x00,0x00,&value, portMAX_DELAY)){
      pread02 = int32_t(value) ;
    vTaskDelay(1 ); 
    }
     }
}

void task1(void *pvParameters)    // sender 1
{
delay(7000);  // wait for pressure sensor initialization at boot
   while(1){
    ppread01 = scale1.get_units();
    xTaskNotify(task_2_handle, ppread01 ,eSetValueWithOverwrite); 
    // }
    vTaskDelay(1); 
    }
}

void task3(void *pvParameters)    // sender 2
{
delay(7000);   // wait for pressure sensor initialization at boot
  while(1){
    delay(1);
    ppread02 = scale2.get_units();
    xTaskNotify(task_4_handle, ppread02 ,eSetValueWithOverwrite); 
    vTaskDelay(1); 
     }
}
...
// long pressure; //
// long read_pressure_pin(int pinToMeasure)   // "stop the world" function
// {
//   // case 32 and case 33 are configured pins for dot and dash
//   switch (pinToMeasure)
//   {
//   case 32:
//     pressure = scale1.get_value();
//     return pressure;
//     break;
//   case 33:
//     pressure = scale2.get_value();
//     return pressure;
//     break;
//   }
//   // return pressure;
...
int paddle_pin_read(int pin_to_read)
{
...
#ifndef FEATURE_PRESSURE_PADDLES //  previous chapter to be replaced with following lines !FEATURE_CAPACITIVE_PADDLE_PINS   FEATURE_PRESSURE_PADDLES
#ifndef OPTION_INVERT_PADDLE_PIN_LOGIC
#ifdef OPTION_DIRECT_PADDLE_PIN_READS_MEGA // after April 2019, if this option is not defined then a direct read of the pins can never occur
  switch (pin_to_read)
  {
...
#if !defined(OPTION_DIRECT_PADDLE_PIN_READS_UNO) && !defined(OPTION_DIRECT_PADDLE_PIN_READS_MEGA) && !defined(OPTION_SAVE_MEMORY_NANOKEYER)
  // delay(500);
  return digitalRead(pin_to_read); // code using ordinary digitalRead
#endif                             // !defined(OPTION_DIRECT_PADDLE_PIN_READS_UNO) && !defined(OPTION_DIRECT_PADDLE_PIN_READS_MEGA)
#else                              // !OPTION_INVERT_PADDLE_PIN_LOGIC
  return !digitalRead(pin_to_read); // invert pin we do the regular digitalRead() if none of the direct register read options are valid
#endif                             // !OPTION_INVERT_PADDLE_PIN_LOGIC
#else                              // !FEATURE_CAPACITIVE_PADDLE_PINS
  ...
  { //  if set to 1 then physical paddles take precedence, pressure paddles ignored
    ...
    {
     #if defined(TIMING_PRESSURE_PADDLES)
      sensor_press_time = esp_timer_get_time();
      iteration = iteration + 1;
      if (sensor_press_time - lastMillis > 10000000)
      {
        Serial.print(F("Loops last 10 second, physical paddle: "));
        Serial.println(iteration);
        lastMillis = sensor_press_time;
        iteration = 0;
      }
      #endif 
      return digitalRead(pin_to_read);    // digitalRead is 20+ times faster than pressure paddles
    } // end if
...
//  pressure = read_pressure_pin(pin_to_read);    //  using normal sensor read , "stop the world" function
    
  #if defined(TIMING_PRESSURE_PADDLES)
  sensor_read_time2 = esp_timer_get_time();
  sensor_read_time = esp_timer_get_time() - sensor_press_time;
  sumdot = sumdot + sensor_read_time;
  iteration = iteration + 1;
  if (sensor_press_time - lastMillis > 10000000)
  {
    Serial.print(F("\n\r Loops last 10 second (pressure paddles):  "));
    Serial.println(iteration);
    lastMillis = sensor_press_time;
    iteration = 0;
  }
  if (iteration > 5000)
  {
    avg_sensor_ms = (sumdot / iteration);
    primary_serial_port->print(F("   \n\r  avg_sensor  "));
    primary_serial_port->print(avg_sensor_ms);
    avg_sensor_ms = 0;
    sumdot = 0;
    iteration = 0;
  }
  #endif // TIMING_PRESSURE_PADDLES

// using FreeRTOS tasks
  if ( (abs(pread01) > configuration.pressure_threshold_dot) && (pin_to_read == 32)  ) 
  {
    return LOW;
  }
  else if ( ( abs(pread02) > configuration.pressure_threshold_dash)   && (pin_to_read == 33)) 
  {
    return LOW;
  }
  else
  ...
  // using  sensor read without freeRTOS
  // if ((pressure > configuration.pressure_threshold_dot) && (pin_to_read == 32))
  // {
  //   return LOW;
  // }
  // else if ((pressure > configuration.pressure_threshold_dash) && (pin_to_read == 33))
  // {
  //   return LOW;
  // }
  // else
...
void initialize_pressure_paddles()
{
#if defined(FEATURE_PRESSURE_PADDLES)

  HX711 scale1;
  HX711 scale2;
  long read01; // initial average  first sensor
  long read02; // initial average  second sensor
  scale1.begin(LOADCELL_DOUT_PIN, LOADCELL_SCK_PIN, true);   //  added "true" for fast CPU
  scale2.begin(LOADCELL2_DOUT_PIN, LOADCELL2_SCK_PIN, true); // pins 13,12,14,27 do not cause crash
                                                       
  long reading = scale2.read();
  primary_serial_port->print("   scale2 reading: ");
  primary_serial_port->print(reading);
  reading = scale1.read();
  primary_serial_port->print("  scale1 reading: ");
  primary_serial_port->print(reading);
  // first sensor
  scale1.tare();          // reset the scale to 0
  scale1.set_scale(9900); // 1kg sensor "scale" is 989    // 100 gram sensor "scale" is 17678 // values changed for esp32
  scale1.tare();          // reset the scale to 0
  read01 = scale1.get_units(20);
  // // second sensor
  scale2.tare();          // reset the scale to 0
  scale2.set_scale(9900); // 1kg sensor "scale" is 989    // 100 gram sensor "scale" is 17678  // values changed for esp32
  scale2.tare();          // reset the scale to 0
  read02 = scale2.get_units(20);

  primary_serial_port->print(F("\n\r sensor1 empty weight (grams): "));
  primary_serial_port->print(read01);
  primary_serial_port->print(F("  sensor2 empty weight (grams): "));
  primary_serial_port->println(read02);
  
primary_serial_port->print("   scale2 get_scale: ");
  primary_serial_port->print(scale2.get_scale());
  primary_serial_port->print("  scale1 get_scale: ");
  primary_serial_port->println(scale1.get_scale());

  
  primary_serial_port->print("   scale2 get_value: ");
  primary_serial_port->print(scale2.get_value(2));
  primary_serial_port->print("  scale1 get_value: ");
  primary_serial_port->println(scale1.get_value(2));

  primary_serial_port->print(F("\n\r finished 'initialize_pressure_paddles' line  "));
  primary_serial_port->print(__LINE__);
...
#if defined(FEATURE_BLUETOOTH)
  // starting BT before initialize_serial_ports
  SerialBT.begin("hb9txb_secondary_serial"); // Bluetooth device name
  delay(100);
  if (SerialBT.available())
  {
    primary_serial_port->print(F("\n\r SerialBT started before initialize serial ports \n\r "));
  }
#endif
...
#if defined(FEATURE_PRESSURE_PADDLES)                // initialize pressure sensors
  scale1.begin(LOADCELL_DOUT_PIN, LOADCELL_SCK_PIN, true); //
  delay(1);
  scale2.begin(LOADCELL2_DOUT_PIN, LOADCELL2_SCK_PIN, true ); //
  scale1.tare();
  scale1.set_scale(9900);
  scale1.tare();
  delay(1);
  scale2.tare();
  scale2.set_scale(2500); // end of to be global
  scale2.tare();
  primary_serial_port->print(F("\n\r hx711 defined at line "));
  primary_serial_port->print(__LINE__);
  primary_serial_port->print(F("\n\r "));
...
// TaskHandle_t task_4_handle;
 ...
 xTaskCreate(task1, "send_notif_1", 8192,  NULL, tskIDLE_PRIORITY,  NULL );
 xTaskCreate(task2, "rec_notif_1", 8192,  NULL,tskIDLE_PRIORITY , &task_2_handle );
 xTaskCreate(task3, "send_notif_2", 8192,  NULL, tskIDLE_PRIORITY,  NULL );
 xTaskCreate(task4, "rec_notif_2", 8192,  NULL, tskIDLE_PRIORITY, &task_4_handle );


// pinning tasks to core does not bring improvement
//  xTaskCreatePinnedToCore(task1, "send_notif_1", 8192,  NULL, configMAX_PRIORITIES - 4 ,  NULL , 1);      // 0
//  xTaskCreatePinnedToCore(task2, "rec_notif_1", 8192,  NULL,configMAX_PRIORITIES - 4  , &task_2_handle , 0);     // 1
//  xTaskCreatePinnedToCore(task3, "send_notif_2", 8192,  NULL, configMAX_PRIORITIES - 3 ,  NULL , 1);            //0
//  xTaskCreatePinnedToCore(task4, "rec_notif_2", 8192,  NULL, configMAX_PRIORITIES - 3 , &task_4_handle , 0);    //1
  ...
  if (SerialBT.available())
  {
    primary_serial_port->print(F("\n\r SerialBT started line after intialize pressure \n\r "));
  }
#endif
  ...
  // initialize_analog_button_array();  //############  COMMENT THIS FEATURE (analog_button_array) UNLOCKED COMMAND LINE AND SOUNT ############
#if !defined(DEBUG_EEPROM_READ_SETTINGS)
  initialize_serial_ports();
#endif
...
#ifdef OPTION_BEACON_MODE_MEMORY_REPEAT_TIME
      if (time_to_delay > 0)
        delay(time_to_delay); // this provdes a delay between succesive playouts of the memory contents
#endif                        // OPTION_BEACON_MODE_MEMORY_REPEAT_TIME
